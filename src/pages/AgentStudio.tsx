...
const AGENTS_DATA = [
  { id: 'lex', name: 'Lex', ini: 'LX', role: 'Jurisdicciones', c: '#8B5CF6', bg: 'linear-gradient(135deg,#5B21B6,#4C1D95)', tasks: 43, active: false, agentType: 'jurisdiction', desc: 'Experto en 200 oficinas PI del mundo. Plazos, tasas y jurisprudencia actualizada.', caps: ['EUIPO, USPTO, OEPM, WIPO', 'Plazos y tasas al día', 'Jurisprudencia por país', 'Estrategia internacional'] },
  { id: 'archie', name: 'Archie', ini: 'AR', role: 'Expedientes', c: '#0EA5E9', bg: 'linear-gradient(135deg,#0369A1,#0284C7)', tasks: 61, active: true, agentType: 'dossier', desc: 'Analiza expedientes en profundidad. Identifica riesgos y recomienda acciones.', caps: ['Análisis de OA', 'Detección de riesgos', 'Historial completo', 'Recomendaciones'] },
  { id: 'draft', name: 'Draft', ini: 'DR', role: 'Documentos', c: '#F59E0B', bg: 'linear-gradient(135deg,#B45309,#DC2626)', tasks: 28, active: false, agentType: 'document', desc: 'Genera documentos legales con evaluación automática de calidad. Sonnet + Haiku en loop.', caps: ['Respuestas a OA', 'Escritos de oposición', 'Contratos de licencia', 'Cartas C&D'] },
  { id: 'scout', name: 'Scout', ini: 'SC', role: 'Inteligencia', c: '#10B981', bg: 'linear-gradient(135deg,#065F46,#059669)', tasks: 17, active: false, agentType: 'competitor', desc: 'Monitoriza el mercado y analiza similitud de marcas. Mapea cobertura territorial.', caps: ['Análisis de similitud', 'Vigilancia competidores', 'Detección infracciones', 'Mapas de cobertura'] },
  { id: 'iris', name: 'Iris', ini: 'IR', role: 'Comunicaciones', c: '#6366F1', bg: 'linear-gradient(135deg,#3730A3,#4F46E5)', tasks: 34, active: false, agentType: 'communication', desc: 'Redacta comunicaciones en el estilo de cada abogado. Aprende con el uso.', caps: ['Emails en tu estilo', 'Instrucciones a agentes', 'Traducciones PI', 'Propuestas comerciales'] },
  { id: 'sage', name: 'Sage', ini: 'SG', role: 'Cartera', c: '#14B8A6', bg: 'linear-gradient(135deg,#0F766E,#0D9488)', tasks: 12, active: false, agentType: 'portfolio', desc: 'Estratega de cartera PI. Maximiza el valor e identifica oportunidades de licencia.', caps: ['Valoración de activos', 'Gaps de cobertura', 'Estrategia de licencias', 'Due diligence'] },
]

// ─── SVG símbolos por agente ───────────────────────────────
function AgentSymbol({ agentId, color }: { agentId: string; color: string }) {
  const c = color
  if (agentId === 'lex') return (
    <svg width="100%" height="106" viewBox="0 0 126 106" fill="none" style={{ position: 'absolute', inset: 0 }}>
  ...
  if (agentId === 'sage') {
    ...
    return (
      <svg width="100%" height="106" viewBox="0 0 126 106" fill="none" style={{ position: 'absolute', inset: 0 }}>
        {hs.map((h, i) => (
          <rect key={i} x={8 + i * 11.5} y={96 - h} width="8" height={h} rx="2" fill={c} opacity={0.2 + (h / 60) * 0.7}
...
// ─── AgentCard ─────────────────────────────────────────────
function AgentCard({
  agent, tasks, active, delay, onActivate, onClick
}: {
  agent: AgentData; tasks: number; active: boolean;
  delay: number; onActivate: () => void; onClick: () => void
}) {
  ...
  return (
    <div
      ...
      style={{
        background: '#07112A',
        border: `1.5px solid ${active ? agent.c : '#0D1C35'}`,
        borderRadius: 14,
        padding: 0,
        cursor: 'pointer',
        position: 'relative',
        ...
        boxShadow: active ? `0 0 28px -6px ${agent.c}33` : 'none',
      }}
      onMouseEnter={e => { if (!active) (e.currentTarget as HTMLDivElement).style.borderColor = agent.c }}
      onMouseLeave={e => { if (!active) (e.currentTarget as HTMLDivElement).style.borderColor = '#0D1C35' }}
    >
      {/* Top glow */}
      {active && (
        <div style={{
      ...
      <div style={{ position: 'relative', height: 106, overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            border: `2px solid ${active ? agent.c : '#0D1C35'}`,
            boxShadow: active ? `0 0 10px ${agent.c}44` : 'none',
          }}>
            {agent.ini}
            <div style={{
...
export function AgentStudio() {
  ...
  const [selectedAgent, setSelectedAgent] = useState<(AgentData & { caps: string[] }) | null>(null)
  ...
  const drawConns = () => {
    ...
    agents.forEach((a, i) => {
      ...
      if (a.active) {
        ...
        ln.setAttribute('stroke', `url(#g${i})`); ln.setAttribute('stroke-width', '2.5')
        ln.setAttribute('stroke-dasharray', '9 5'); ln.setAttribute('opacity', '1')
        ln.style.animation = 'cf 1.1s linear infinite'
        svg.appendChild(ln)
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      if (!idle.length || Math.random() > 0.62) return
      const pick = idle[Math.floor(Math.random() * idle.length)]
      setAgents(prev => prev.map(a => a.id === pick.id ? { ...a, active: true } : a))
      setTimeout(() => {
        setAgents(prev => prev.map(a => a.id === pick.id ? { ...a, active: false } : a))
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 0, position: 'relative', zIndex: 2 }}>
          <div ref={nexusRef} onClick={() => setSelectedAgent(NEXUS_DATA as any)} style={{
      ...
      {selectedAgent && (
        ...
        >
          <div style={{
            background: '#0A1628', border: '1px solid #172840', borderRadius: 16,
            padding: 28, width: 360, maxHeight: '80vh', overflow: 'auto',
            position: 'relative', animation: 'land .4s cubic-bezier(.22,1,.36,1)',
          }}>
            <button onClick={() => setSelectedAgent(null)} style={{
              position: 'absolute', top: 11, right: 12, background: '#05090F',
              border: '1px solid #0D1C35', color: '#1E3450', cursor: 'pointer',
              width: 24, height: 24, borderRadius: '50%', fontSize: 12,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>✕</button>
            <div style={{
              width: 48, height: 48, borderRadius: '50%', background: selectedAgent.bg,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 16, fontWeight: 900, color: '#fff', margin: '0 auto 12px',
            }}>{selectedAgent.ini}</div>
            <div style={{ fontSize: 18, fontWeight: 800, textAlign: 'center', marginBottom: 2 }}>{selectedAgent.name}</div>
            <div style={{ fontSize: 11, color: '#64748B', textAlign: 'center', marginBottom: 12 }}>{selectedAgent.role}</div>
            <p style={{ fontSize: 12, color: '#94A3B8', lineHeight: 1.6, marginBottom: 16 }}>{selectedAgent.desc}</p>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#64748B', marginBottom: 8 }}>Capacidades</div>
            {selectedAgent.caps.map((cap: string, i: number) => (
              <div key={i} style={{
                display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6,
                fontSize: 11, color: '#94A3B8',
              }}>
                <div style={{
                  width: 5, height: 5, borderRadius: '50%', background: selectedAgent.c, flexShrink: 0,
                }} />
                {cap}
              </div>
            ))}
            <div style={{ display: 'flex', gap: 12, marginTop: 16, marginBottom: 16 }}>
              {[
                { v: selectedAgent.tasks, l: 'tareas hoy', c: selectedAgent.c },
                { v: '98%', l: 'tasa éxito', c: '#22C55E' },
              ].map(({ v, l, c }) => (
                <div key={l} style={{
                  flex: 1, background: '#05090F', borderRadius: 10, padding: '10px 12px', textAlign: 'center',
                }}>
            ...
            <button
              onClick={() => {
                ...
                window.dispatchEvent(new CustomEvent('open-copilot-with-agent', { detail: { agent: selectedAgent.id, name: selectedAgent.name } }))
              }}
              style={{
                width: '100%', border: 'none', borderRadius: 10, color: '#fff',
                fontSize: 12, fontWeight: 700, padding: 11, cursor: 'pointer',
                background: selectedAgent.bg, fontFamily: 'inherit',
              }}
            >
