      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div
            ...
            <h2 style={{ fontSize: '36px', fontWeight: 700, color: '#14213D', letterSpacing: '-0.03em', lineHeight: 1.15 }}>
              Tu dinero está seguro.
              <br />
              <span style={{ color: '#10B981' }}>Siempre.</span>
            </h2>
            <p className="mt-5" style={{ fontSize: '15px', color: '#475569', lineHeight: 1.8, maxWidth: '460px' }}>
          ...
          <motion.div
            ...
            style={{ background: '#fff', boxShadow: '0 1px 3px rgba(20,33,61,0.04), 0 8px 40px rgba(20,33,61,0.06)' }}>
                ...
                <div key={idx} className="flex items-start gap-4 relative">
                  <div className="flex flex-col items-center shrink-0">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg"
                      style={{
                        background: step.status === 'done' ? '#ECFDF5' : step.status === 'active' ? 'rgba(252,163,17,0.1)' : '#F8FAFC',
                        border: step.status === 'active' ? '2px solid #FCA311' : 'none',
                      }}>
                      {step.icon}
                    </div>
                  ...
                  <div className="pt-1.5 pb-4 flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span style={{ fontSize: '14px', fontWeight: 600, color: step.status === 'pending' ? '#94A3B8' : '#14213D' }}>
                        {step.text}
                      </span>
                      <span style={{ fontSize: '14px', fontWeight: 700, color: step.status === 'done' ? '#10B981' : step.status === 'active' ? '#FCA311' : '#94A3B8' }}>
                        {step.amount}
                      </span>
                    </div>
